...
The default output directory is `./reports`. The interactive tree supports arrow-key or Vim navigation (`hjkl`), Space to toggle a project, `B` to toggle a whole branch, `A` to select all, `N` to clear selection, `/` to search, Enter to scan, and `Q` to cancel. For unattended runs, use `--all` or `--profile` and add `--no-open` to skip launching a browser. A profile supplies the project roots and cannot be combined with root arguments or `--all`.
